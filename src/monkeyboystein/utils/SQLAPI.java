...
public class SQLAPI {
    ...
    public int getGems(String name)
    {
        ...
        try
        {
            ...
            String query = "SELECT * FROM " + storage.getDatabase() + ".players";
...
//            String rank = rs.getString("rank");
    ...
    public void addPurchase(String purchase, String playername)
    {
        ...
        try
        {
            ...
            statement.executeUpdate("UPDATE `" + storage.getDatabase() + "`.`purchases` SET `purchases`='" + purchases + "' WHERE `playerName`='" + p.getName() + "';");
            statement.close();
    ...
    public String getPurchases(String player)
    {
        try
        {
            ...
            String query = "SELECT * FROM " + storage.getDatabase() + ".purchases";
            // create the java statement
            Statement st = con.createStatement();
            // execute the query, and get a java resultset
            ResultSet rs = st.executeQuery(query);
    ...
    public void setRank(String name, String rank)
    {
        try
        {
            ...
            statement.executeUpdate("UPDATE `" + storage.getDatabase() + "`.`players` SET `rank`='" + i + "' WHERE `playerName`='" + p.getName() + "';");
            statement.close();
            //  sender.sendMessage(ChatColor.BLUE + "Set %player%'s coins to %i%".replaceAll("%player%",p.getName()).replaceAll("%i%",i + ""));
        }catch (Exception ex){}
    }
    public void setGems(String name, int gems)
    {
        try
        {
            ...
            statement.executeUpdate("UPDATE `" + storage.getDatabase() + "`.`players` SET `gems`='" + i + "' WHERE `playerName`='" + p.getName() + "';");
            statement.close();
            // sender.sendMessage(ChatColor.BLUE + "Set %player%'s coins to %i%".replaceAll("%player%",p.getName()).replaceAll("%i%",i + ""));
        }catch (Exception ex){}
    }
    ...
    public void insert(String player, int gems)
    {
        ...
        try {
            ...
            st.executeUpdate("INSERT INTO `" + storage.getDatabase() + "`.`players` (`playerName`, `gems`, `rank`)" + valuess);


            valuess = " VALUES ( '" + player+ "', 'Default');";
            st.executeUpdate("INSERT INTO `" + storage.getDatabase() + "`.`purchases` (`playerName`,`purchases`)" + valuess);
            st.close();
...
//            System.out.print(ex.getCause());
  ...
  PRIMARY KEY (`playerName`));
         */
        Statement st = con.createStatement();
        st.executeUpdate("CREATE TABLE `" + storage.getDatabase() + "`.`players` (" +
                "  `playerName` VARCHAR(17) NOT NULL," +
                "  `gems` INT NOT NULL," +
                "  `rank` VARCHAR(45) NOT NULL," +
                "  PRIMARY KEY (`playerName`));");
        ...
        st.executeUpdate("CREATE TABLE `" + storage.getDatabase() + "`.`purchases` (" +
                "  `playerName` VARCHAR(17) NOT NULL," +
